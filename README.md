...
| `get_node_ancestry`            | Get the ancestry chain of a node up to root page, containing screen/artboard & variants  |
| `find_instances`               | Find all instances of a component or component set across screens without tree dumps     |
...
| `save_screenshots`             | Export and save screenshots directly to filesystem (supports absolute paths & overwrite) |
...
| `set_node_properties`          | Patch name, bounds, visibility, opacity, corner radius, constraints, and layout sizing   |
...
| `create_instance`              | Create an instance (❖ -> ◇) of a component or component set under a parent frame         |
| `reset_instance_overrides`     | Reset all overrides on an instance or child node back to master component                |
| `import_html_layers`           | Bulk-import an html-figma layer tree (JSON) as frames, text, rectangles, and vectors     |
| `duplicate_nodes`              | Duplicate nodes in place (supports `asInstance: true` for components)                    |
